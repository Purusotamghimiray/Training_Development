...
class hotel_folio(models.Model):
    ...
    childs = fields.Integer(string="Children")
    ...
    def write(self,vals):
        ...
        if self._context.get('active_id'):
            folio_obj = self.browse(self._context.get('active_id'))
        elif self._context.get('params'):
            folio_obj = self.browse(self._context.get('params').get('id'))
        else:
            folio_obj = self
        no_of_adults_room = 0
        no_of_child_room = 0
        for rec in folio_obj.room_lines:
            if rec.product_id:
                room_id = self.env['hotel.room'].search([('product_id','=',rec.product_id.id)])
                no_of_adults_room += room_id.max_adult
                no_of_child_room += room_id.max_child

        if folio_obj.adults <= 0:
            raise UserError(
                'Please set no of adults for reservation')

        if folio_obj.adults > no_of_adults_room:
            raise UserError('You can not exceed the number of adults than the set limit on '
                                    'individual room. Please select additional room')

        if folio_obj.childs > no_of_child_room:
            raise UserError('You can not exceed the number of children than the set limit on '
                                    'individual room. Please select additional room')
        return res
    ...
    def action_confirm(self):
        ...
        for record in self:
            ...
            for line in record.room_lines:
                ...
                _logger.info("ROOM HISTORY ID====>>>>>{}".format(room_his_id))
                if room_his_id:
...
class sale_order_line(models.Model):
    ...
    def _get_to_invoice_qty(self):
        ...
        for line in self:
            print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$4",self._context)
            if line.order_id.state in ['sale', 'done','progress']:
                print("@@@@@@@@@@@@@@@@@@@@@@@@@",line.order_id.state)
                if line.product_id.invoice_policy == 'order':
                    line.qty_to_invoice = line.product_uom_qty - line.qty_invoiced
                    line.invoice_status = 'to invoice'
                    line.order_id.invoice_status = 'to invoice'
                    print("QTY TO INVOICE===================",line.qty_to_invoice)
                else:
    ...
    def _compute_invoice_status(self):
        ...
        for line in self:
            if line.state not in ('sale', 'done', 'progress'):
                line.invoice_status = 'no'
                line.order_id.invoice_status = 'no'
            elif line.is_downpayment and line.untaxed_amount_to_invoice == 0:
                line.invoice_status = 'invoiced'
                line.order_id.invoice_status = 'invoiced'
