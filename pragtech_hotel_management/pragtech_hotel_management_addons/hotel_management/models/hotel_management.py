...
class hotel_reservation(models.Model):
    ...
    def _amount_line_tax(self, line):
        val = 0.0
        # print("LINE NUMBER OF DAYS===>>>",line.number_of_days)
        if line.number_of_days == 0:
            num_of_days = 1
        else:
            num_of_days = line.number_of_days
        taxes = line.taxes_id.compute_all(
            line.price * (1 - (line.discount or 0.0) / 100.0), quantity=num_of_days)
        # print("TAXES===>>>>>>>>>>>",taxes)
        val = taxes['total_included'] - taxes['total_excluded']
    ...
    def _get_total_rental_cost(self):
        ...
        for order in self:
            ...
            cur = order.pricelist_id.currency_id
            for line in order.reservation_line:
                # print("TOTAL===>>>>>>>>",line.sub_total1)
                # print("AMOUNT TAX LINE===>>>>>",self._amount_line_tax(line))
                total1 += line.sub_total1
                val1 += self._amount_line_tax(line)
    ...
    def compute(self):
        ...
        # print("SUM HOTEL RESERVATION===>>>>",sum)
        ...
        for reservation in self:
            ...
            for line in reservation.reservation_line:
                ...
                vals = {
                    ...
                    'qty_to_invoice': line.number_of_days,
                    'checkin_date': line.checkin,
                    'checkout_date': line.checkout,
...
class hotel_reservation_line(models.Model):
    ...
    def onchange_date_count_total_days(self):
        _logger.info("SELF====>>>>>>{}".format(self.line_id))
        shop_id = None
        if not self.checkin:
            return False
        ...
        if day_count2 == 0:
            day_count2 = 1
        _logger.info("DAY COUNT22===>>>>>111111111111>>>>{}".format(day_count2))
        self.number_of_days = day_count2
        if self.line_id.state == 'done':
            self.write({
                'number_of_days': day_count2
            })
...
class hotel_folio_line(models.Model):
    ...
    def onchange_product_id(self):
        ...
        if self.product_id:
            ...
            v['invoice_status'] = 'to invoice'
            print("PRODUCT UOM QTY===================",self.product_uom_qty)
            v['qty_to_invoice'] = self.product_uom_qty
        return {'value': v}
...
class product_template(models.Model):
    ...
    def on_change_shop_id(self):
        # print("SELF===>>>>>>>>>>",self)
        # print("ON CHAGE SHOP ID===>>>>>>>>>>>>>>>>>>>>>>>>>>",self.shop_id)
        if not self.shop_id:
            return {'value': {}}
